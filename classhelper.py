...
if "levels" not in st.session_state:
    st.session_state.levels = DEFAULT_LEVELS.copy()
...
with st.sidebar:
    ...
    # ============================================================
        ...
        if len(st.session_state.levels) == 0:
            ...
            with col_delete:
                if st.button("🗑️ 删除此关卡", use_container_width=True):
    ...
    st.markdown("---")
    st.markdown("**📊 当前关卡数**：{}".format(len(st.session_state.levels)))
...
if levels_count == 0:
    # 无关卡时显示提示
    st.info("📭 **暂无题目**，请在左侧「题目管理」中添加关卡！")
    st.markdown("---")
    st.markdown("👈 点击侧边栏「➕ 添加新关卡」开始创建题目！")
